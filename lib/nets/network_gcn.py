...
from conv.gmm_conv import GMMConv
...
class Network(nn.Module):
    ...
    def _add_losses(self, sigma_rpn=3.0):
        ...
        # print("cls_score = {}, {}".format(cls_score.shape, cls_score.data))
        label = self._proposal_targets["labels"].view(-1)
        # print("label = {}, {}".format(label.shape, label.data))
    ...
    def _init_modules(self):
        ...
        self.new_cls_score_net = nn.Linear(self._fc7_channels + 256, self._num_classes)
        self.new_bbox_pred_net = nn.Linear(self._fc7_channels + 256, self._num_classes * 4)
    ...
    def init_weights(self):
      ...
      """
        ...
        normal_init(self.relation_fc_1, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.relation_fc_2, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.sg_conv_1, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.sg_conv_2, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.new_cls_score_net, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.new_bbox_pred_net, 0, 0.001, cfg.TRAIN.TRUNCATED)
    ...
    def load_state_dict(self, state_dict):
        print("load_state_dict in network_gcn")

        model_dict = nn.Module.state_dict(self)
    ...
    #         )
    #
    # def load_baseline_dict(self, state_dict):
    #     print("load_baseline_dict in network_gcn")
    #
    #     model_dict = nn.Module.state_dict(self)
    #     model_dict.update(state_dict)
    #     # for k, v in model_dict.items():

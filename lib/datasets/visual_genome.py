...
class visual_genome(imdb):
    def __init__(self, image_set, use_diff=False):
        name = 'visual_genome_' + image_set
        if use_diff:
            name += '_diff'
        imdb.__init__(self, name)
        self._image_set = image_set
        self._devkit_path = self._get_default_path()
        self._data_path = self._devkit_path #os.path.join(self._devkit_path, 'VOC' + self._year)

        class_list = ['__background__']
        f = open(os.path.join(cfg.DATA_DIR, "vg", "objects_list.txt"))
        lines = f.readlines()
        for line in lines:
            obj = line.strip()
        ...
        # print(class_list)
        # print(tuple(class_list))
        self._classes = tuple(class_list)
        ...
        assert os.path.exists(self._devkit_path), \
          'Visual Genome path does not exist: {}'.format(self._devkit_path)
    ...
    """
        ...
        for ix, obj in enumerate(objs):
            ...
            if classname in self._class_to_ind:
    ...
    def _get_voc_results_file_template(self):
        ...
        path = os.path.join(self._devkit_path, 'results', 'visual_genome',
                            'Main', filename)
        return path

    def _write_voc_results_file(self, all_boxes):
        print(len(all_boxes), len(all_boxes[0]), len(self.classes), len(self._image_index))
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print('Writing {} Visual Genome results file'.format(cls))
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        imagesetfile = os.path.join(self._devkit_path,
                                    'ImageSets', 'Main',
                                    self._image_set + '.txt')
        # print("annopath = {}, imagesetfile= {}".format(annopath, imagesetfile))
